    ...
    def test_item_returns_metadata(self, client):
        mock_item = {
            ...
            "data": {
                "title": "Test Paper",
                "creators": [
                    {"lastName": "Smith", "firstName": "J", "creatorType": "author"}
                ],
...
class TestBibtexEndpoint:
    def test_bibtex_returns_entry(self, client):
        with (
            patch("riszotto.api.routes.get_client", return_value=MagicMock()),
            patch(
                "riszotto.api.routes.get_item_bibtex",
                return_value="@article{ABC123, title={Test}}",
            ),
        ):
...
class TestStatusEndpoint:
    def test_status_returns_index_info(self, client):
        ...
        with (
            ...
            patch(
                "riszotto.api.routes.get_index_status",
                return_value={"count": 100, "path": "/tmp"},
            ),
        ):
